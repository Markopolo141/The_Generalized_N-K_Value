...
@click.argument('finish_seconds', type=click.INT)
def run(input_file, output_file, thread_number, finish_seconds):
	print "---START---"
	global N
	...
	t = time.time()
	while (time.time()-t < finish_seconds):
		time.sleep(0.5)
		...
		print "{}: relative magnitude error: {}".format(int(finish_seconds-time.time()+t),magerror)
